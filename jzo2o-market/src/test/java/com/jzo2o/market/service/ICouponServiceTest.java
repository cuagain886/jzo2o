...
public class ICouponServiceTest {
    ...
    void test_seizeCouponScriptLua() {
        ...
        //活动id
        Long activityId = 1854795083811938304L;
        int index = (int) (activityId % 10);
        //用户id
        Long userId = 1716346406098296832L;

        //准备keys
        //抢券同步队列  QUEUE:COUPON:SEIZE:SYNC:{活动id%10}
        String couponSeizeSyncRedisKey = RedisSyncQueueUtils.getQueueRedisKey(COUPON_SEIZE_SYNC_QUEUE_NAME, index);
        //库存key
        String couponResourceStockRedisKey = String.format(COUPON_RESOURCE_STOCK, index);
        //抢券成功列表key
        String couponSeizeListRedisKey = String.format(COUPON_SEIZE_LIST, activityId, index);

        redisTemplate.opsForHash().put(couponResourceStockRedisKey, activityId, 100);

        List<String> keys = Arrays.asList(couponSeizeSyncRedisKey, couponResourceStockRedisKey, couponSeizeListRedisKey);
