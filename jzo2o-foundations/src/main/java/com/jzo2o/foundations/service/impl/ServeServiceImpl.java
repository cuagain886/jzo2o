...
public class ServeServiceImpl extends ServiceImpl<ServeMapper, Serve> implements IServeService {
    ...
    public void batchAdd(List<ServeUpsertReqDTO> serveUpsertReqDTOList) {
        for (ServeUpsertReqDTO serveUpsertReqDTO : serveUpsertReqDTOList) {
            ...
            //布隆过滤器
            homeBloomFilterManager.putServeId(serve.getId());
        }
    }

    private void refreshServeCache(Long id,Serve result){
        String cacheKey=RedisConstants.CacheName.SERVE+"::"+id;
        if(result!=null&&FoundationStatusEnum.ENABLE.getStatus()==result.getSaleStatus()){
    ...
    public Serve update(Long id, BigDecimal price) {
        ...
        Serve result = baseMapper.selectById(id);
        refreshServeCache(id, result);
    ...
    public Serve onSale(Long id){
       ...
       //更新上架状态
        ...
        refreshServeCache(id, result);
        //更新布隆过滤器
        homeBloomFilterManager.putServeId(id);
        return baseMapper.selectById(id);
