...
public class ServeItemServiceImpl extends ServiceImpl<ServeItemMapper, ServeItem> implements IServeItemService {
    ...
    private void refreshServeItemCache(Long id, ServeItem result) {
        String cacheKey = RedisConstants.CacheName.SERVE_ITEM + "::" + id;
        if (result != null && FoundationStatusEnum.ENABLE.getStatus()==result.getActiveStatus()) {
            cacheWriteHelper.put(cacheKey, result, Duration.ofDays(1), TimeUnit.HOURS.toSeconds(2));
            return;
        }
        cacheWriteHelper.delete(cacheKey);
    }
    /**
     * 服务项新增
     *
    ...
    public void add(ServeItemUpsertReqDTO serveItemUpsertReqDTO) {
        ...
        homeBloomFilterManager.putServeItemId(serveItem.getId());
    }
    ...
    public ServeItem update(Long id, ServeItemUpsertReqDTO serveItemUpsertReqDTO) {
        ...
        ServeItem result = baseMapper.selectById(id);
        refreshServeItemCache(id, result);
    ...
    public ServeItem activate(Long id) {
        ...
        homeBloomFilterManager.putServeItemId(id);
